...
void sccp_linedevice_deleteButtonsArray(devicePtr device)
{
	...
	/* Capture the length together with the pointer it belongs to, before the claim.
	 * Reading it afterwards could pick up the length of a replacement array that a
	 * concurrent createButtonsArray installed in the meantime, and then walk this
	 * (shorter) array past its end. */
	uint8_t size = device->lineButtons.size;

	/* On builds without SCCP_ATOMIC, CAS_PTR degrades to a trylock that is allowed
	 * to fail spuriously on contention. Failing out of a cleanup path would leak the
	 * array and every linedevice reference it still holds, so retry instead of
	 * giving up - but stop once another thread has claimed the pointer itself. */
	while(!CAS_PTR(&device->lineButtons.instance, instances, NULL, &device->messageStack.lock)) {
		if(device->lineButtons.instance != instances) {
			return;                                                      /* claimed by someone else */
		}
	}

	/* Clear the length only while no replacement array has been installed yet;
	 * otherwise this would zero the length of an array that is still in use. */
	if(!device->lineButtons.instance) {
