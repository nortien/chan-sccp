...
void sccp_rtp_print(constChannelPtr c, sccp_rtp_type_t type, struct ast_str * buf, int buflen)
{
	...
	pbx_str_reset(buf);
	/* sccp_netsock_stringify() returns a pointer into a shared thread-local
	 * buffer (ast_str_thread_get) - calling it more than once as separate
	 * arguments to the same format call aliases them all to whichever call
	 * happened to run last. Copy each result out first. */
	if (isNatted) {
