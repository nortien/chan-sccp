...
AC_DEFUN([AST_SET_PBX_AMCONDITIONALS],[
	...
	# The per-version ASTERISK_VER_GROUP_1NN conditionals that used to live here
	# tested ${ASTTERISK_VER_GROUP} - a name that never existed - so every one of
	# them was always false and the AC_CONFIG_FILES calls they guarded never ran.
	# All version-group Makefiles are now registered unconditionally in
	# configure.ac, which is what `make dist` needs in order to walk them.
])
